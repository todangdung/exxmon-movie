...
const MovieVideo = (props) => {
    ...
    useEffect(() => {
        ...
        switch (props.category) {
            case "movie":
                setUrl(`https://vidsrc.me/embed/${props.id}`);
                break;
            case "tv":
                setUrl(
                    `https://vidsrc.me/embed/${props.id}/${season}-${episode}`
                );
                break;
            default:
                setUrl("");
                    ...
                    <>
                        ...
                        <div className="seasons">
                            {props.item.seasons?.map((item, index) => (
                                <div
                                    className="movie-video__info__seasons"
                                    key={index}
                                >
                                    <button
                        ...
                        <div className="episode">
                            {props.item.seasons?.map((item, index) => (
                                <div className="movie-video__info__episode__item">
                                    {index + 1 === season &&
                                        episodeCount(item.episode_count).map(
                                            (item, index) => (
                                                <button
                                                    onClick={() =>
                                                        setEpisode(index + 1)
