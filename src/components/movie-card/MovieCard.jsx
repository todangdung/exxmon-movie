...
const MovieCard = (props) => {
  ...
  useEffect(() => {
    ...
    if (movie) {
      movie.forEach((e) => {
        if (cardRef && cardRef.current && e == parseInt(cardRef.current.id)) {
  ...
  const saveLocal = useCallback(
    (id) => {
      ...
      if (check) {
        localStorage.setItem(props.category, JSON.stringify([...filter]));
        buttonRef.current.firstChild.classList.remove("active");
      } else {
        localStorage.setItem(props.category, JSON.stringify([...movie, id]));
      }
    },
    [props.category]
  );
  ...
  return (
    <div className="movie-card" id={item.id} ref={cardRef} onClick={checkLogin}>
      <Link to={link}>
      ...
      <div className="button" ref={buttonRef}>
        <TransparentButton className="white" onClick={() => saveLocal(item.id)}>
          <h2>
